...
| <b>hexRadius</b> | <i>number</i> | 0.25 | The radius of the hexagons that tesselate the globe's surface. The radius is specified in terms of degrees of the globe's great circle perimeter. |
...
| <b>hexColor</b> | <i>func</i> | `() => '#ffffaa'` | Accessor method for each hexagon's color. The method should follow the signature: `hexColor(({ points, sumWeight, center: { lat, lng }}) => ...)` and return a color string. |
